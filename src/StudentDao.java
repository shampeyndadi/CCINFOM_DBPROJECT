...
public class StudentDao{
    ...
    public void addStudent(Student student){String sqlStatement = "INSERT INTO Students (student_id, first_name, last_name, birth_date, gender, address, email, program_id, phone_number, account_password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Object[][] getStudentSchedule(int studentId) {
        String sql = """
        SELECT c.class_id, co.course_id, CONCAT(i.inst_first_name, ' ', i.inst_last_name) AS instructor,
               DATE_FORMAT(c.schedule, '%W') AS day_of_week,
               DATE_FORMAT(c.schedule, '%h:%i %p') AS start_time,
               DATE_FORMAT(DATE_ADD(c.schedule, INTERVAL 1 HOUR), '%h:%i %p') AS end_time,
               c.room_number
        FROM Enrollments e
        JOIN Classes c ON e.class_id = c.class_id
        JOIN Courses co ON c.course_id = co.course_id
    ...
    """;
        ...
        try (Connection connection = DatabaseConnect.getConnection();
            ...
            while (rs.next()) {
                ...
                row[1] = rs.getString("instructor");
                row[2] = rs.getString("day_of_week");
                row[3] = rs.getString("start_time");
                row[4] = rs.getString("end_time");
                row[5] = rs.getString("room_number");
                scheduleList.add(row);
            }

            Object[][] scheduleData = new Object[scheduleList.size()][7];
    ...
    public static void main(String[] args){
        Student s1 = new Student(12307211, "Jedidiah", "Julian", "charlesxxavier30@gmail.com", 'M', "October 1, 2004", "Cubao QC", "CCS-ST", "jedidiah1001", "09173099783");
