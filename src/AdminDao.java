...
public class AdminDao {
    ...
    public ArrayList<String[]> getInstructorSchedules(int instructorId) {
        ...
        String term = adminDao.getCurrentTerm();
        String schoolYear = adminDao.getCurrentSchoolYear();

        ArrayList<String[]> schedules = new ArrayList<>();
        String sql = """
        SELECT s.scheduled_days, s.class_hours
        FROM Schedules s
    ...
    """;
        ...
        try (Connection conn = DatabaseConnect.getConnection();
            ...
            stmtClass.setString(8, currentTerm);
            stmtClass.setString(9, currentSchoolYear);
    ...
    public String generateUniqueStudentId() {
        ...
        String yearSuffix = years[0].substring(2);
    ...
    public void updateTermAndSchoolYear() {
        String currentTerm = getCurrentTerm();
        String currentSchoolYear = getCurrentSchoolYear();

        if (areClassesStillInProgress(currentTerm, currentSchoolYear)) {
            JOptionPane.showMessageDialog(null,
                    "Classes are still in progress for the current term (" + currentTerm + ") and school year (" + currentSchoolYear + ").",
                    "Update Denied",
        ...
        String newTerm;
        String newSchoolYear = currentSchoolYear;
        ...
        switch (currentTerm) {
            ...
            case "Third Term":
                ...
                newSchoolYear = (startYear + 1) + "-" + (endYear + 1);
                break;
            default:
                JOptionPane.showMessageDialog(null, "Invalid current term: " + currentTerm, "Error", JOptionPane.ERROR_MESSAGE);
                return;
        }

        try (Connection connection = DatabaseConnect.getConnection();
             PreparedStatement updateTermStmt = connection.prepareStatement(
                     "UPDATE SystemSettings SET setting_value = ? WHERE setting_name = 'current_term'");
             PreparedStatement updateSchoolYearStmt = connection.prepareStatement(
                     "UPDATE SystemSettings SET setting_value = ? WHERE setting_name = 'current_school_year'")) {


            updateTermStmt.setString(1, newTerm);
            updateTermStmt.executeUpdate();

            if (!newSchoolYear.equals(currentSchoolYear)) {
                updateSchoolYearStmt.setString(1, newSchoolYear);
                updateSchoolYearStmt.executeUpdate();
            }

            JOptionPane.showMessageDialog(null,
                    "Term updated: " + newTerm + "\nSchool Year: " + newSchoolYear,
                    "Update Successful",
                    JOptionPane.INFORMATION_MESSAGE);

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Database error while updating term and school year: " + e.getMessage(),
                    "Database Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private boolean areClassesStillInProgress(String currentTerm, String currentSchoolYear) {
        String sql = """
        SELECT COUNT(*) AS active_classes
        FROM AcademicHistory ah
        WHERE ah.term = ? AND ah.school_year = ? AND ah.status = 'In Progress';
    """;

        try (Connection connection = DatabaseConnect.getConnection();
             PreparedStatement stmt = connection.prepareStatement(sql)) {

            stmt.setString(1, currentTerm);
            stmt.setString(2, currentSchoolYear);

            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next() && rs.getInt("active_classes") > 0) {
                    return true;
                }
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null,
                    "Database error while checking active classes: " + e.getMessage(),
                    "Database Error",
